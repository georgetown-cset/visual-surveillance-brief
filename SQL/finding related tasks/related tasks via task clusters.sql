...
-- task_family or base_task = umbrella term for the task
...
base_task_papers AS(
SELECT merged_id, raw_task, task_family as base_task
...
SELECT DISTINCT merged_id, cluster_name, task_family as base_task 
FROM base_task_paper_clusters
INNER JOIN task_family_pairs ON(task_family_pairs.raw_task = base_task_paper_clusters.task)
),
...
SELECT * from top_overlapping_tasks
